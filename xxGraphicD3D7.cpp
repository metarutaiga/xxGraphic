...
//------------------------------------------------------------------------------
uint64_t xxCreateVertexBufferD3D7(uint64_t device, unsigned int size, uint64_t vertexAttribute)
{
    LPDIRECT3DDEVICE7 d3dDevice = reinterpret_cast<LPDIRECT3DDEVICE7>(device);
    if (d3dDevice == nullptr)
        return 0;
...
//  Texture
//==============================================================================
uint64_t xxCreateTextureD3D7(uint64_t device, int format, unsigned int width, unsigned int height, unsigned int depth, unsigned int mipmap, unsigned int array, const void* external)
{
    DDSURFACEDESC2 desc = {};
    desc.dwSize = sizeof(DDSURFACEDESC2);
...
//------------------------------------------------------------------------------
void* xxMapTextureD3D7(uint64_t device, uint64_t texture, unsigned int* stride, unsigned int level, unsigned int array)
{
    LPDIRECTDRAWSURFACE7 surface = reinterpret_cast<LPDIRECTDRAWSURFACE7>(texture);
    if (surface == nullptr)
        return nullptr;
...
//------------------------------------------------------------------------------
void xxUnmapTextureD3D7(uint64_t device, uint64_t texture, unsigned int level, unsigned int array)
{
    LPDIRECTDRAWSURFACE7 surface = reinterpret_cast<LPDIRECTDRAWSURFACE7>(texture);
    if (surface == nullptr)
        return;
