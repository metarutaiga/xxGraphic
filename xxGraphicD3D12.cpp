...
uint64_t xxCreateDeviceD3D12(uint64_t instance)
{
    ...
    xxLocalBreak()
    {
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device13*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.13", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device12*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.12", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device11*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.11", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device10*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.10", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device9*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.9", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device8*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.8", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device7*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.7", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device6*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.6", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device5*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.5", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device4*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.4", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device3*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.3", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device2*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.2", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device1*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.1", xxGetInstanceName());
            break;
        }
        if (d3dDevice->QueryInterface(__uuidof(ID3D12Device*), (void**)&unknown) == S_OK)
        {
            xxLog("xxGraphic", "%s %s (%s)", "Direct3D", "12.0", xxGetInstanceName());
            break;
        }
    }
    SafeRelease(unknown);
...
uint64_t xxCreateTextureD3D12(uint64_t device, uint64_t format, int width, int height, int depth, int mipmap, int array, void const* external)
{
    ...
    switch (format)
    {
    case "RGB565"_CC:   T = { DXGI_FORMAT_B5G6R5_UNORM,     D3D12_ENCODE_SHADER_4_COMPONENT_MAPPING(2,1,0,3) }; break;
    case "BGR565"_CC:   T = { DXGI_FORMAT_B5G6R5_UNORM,     D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "ARGB1555"_CC: T = { DXGI_FORMAT_B5G5R5A1_UNORM,   D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "ABGR1555"_CC: T = { DXGI_FORMAT_B5G5R5A1_UNORM,   D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "ARGB4444"_CC: T = { DXGI_FORMAT_B4G4R4A4_UNORM,   D3D12_ENCODE_SHADER_4_COMPONENT_MAPPING(1,0,3,2) }; break;
    case "ABGR4444"_CC: T = { DXGI_FORMAT_B4G4R4A4_UNORM,   D3D12_ENCODE_SHADER_4_COMPONENT_MAPPING(3,0,1,2) }; break;
    case "RGBA4444"_CC: T = { DXGI_FORMAT_B4G4R4A4_UNORM,   D3D12_ENCODE_SHADER_4_COMPONENT_MAPPING(2,1,0,3) }; break;
    case "BGRA4444"_CC: T = { DXGI_FORMAT_B4G4R4A4_UNORM,   D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "RGBA5551"_CC: T = { DXGI_FORMAT_B5G5R5A1_UNORM,   D3D12_ENCODE_SHADER_4_COMPONENT_MAPPING(2,1,0,3) }; break;
    case "BGRA5551"_CC: T = { DXGI_FORMAT_B5G5R5A1_UNORM,   D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "ARGB8888"_CC: T = { DXGI_FORMAT_R8G8B8A8_UNORM,   D3D12_ENCODE_SHADER_4_COMPONENT_MAPPING(1,2,3,0) }; break;
    case "RGBA8888"_CC: T = { DXGI_FORMAT_R8G8B8A8_UNORM,   D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "ABGR8888"_CC: T = { DXGI_FORMAT_B8G8R8A8_UNORM,   D3D12_ENCODE_SHADER_4_COMPONENT_MAPPING(3,0,1,2) }; break;
    case "BGRA8888"_CC: T = { DXGI_FORMAT_B8G8R8A8_UNORM,   D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "DS24"_cc:     T = { DXGI_FORMAT_D24_UNORM_S8_UINT, D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING        }; break;
    case "BC1"_cc:
    case "DXT1"_cc:     T = { DXGI_FORMAT_BC1_UNORM,        D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "BC2"_cc:
    case "DXT3"_cc:     T = { DXGI_FORMAT_BC2_UNORM,        D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "BC3"_cc:
    case "DXT5"_cc:     T = { DXGI_FORMAT_BC3_UNORM,        D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "BC4S"_cc:     T = { DXGI_FORMAT_BC4_SNORM,        D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "BC4U"_cc:
    case "ATI1"_cc:     T = { DXGI_FORMAT_BC4_UNORM,        D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "BC5S"_cc:     T = { DXGI_FORMAT_BC5_SNORM,        D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "BC5U"_cc:
    case "ATI2"_cc:     T = { DXGI_FORMAT_BC5_UNORM,        D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "BC6H"_cc:     T = { DXGI_FORMAT_BC6H_SF16,        D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    case "BC7"_cc:      T = { DXGI_FORMAT_BC7_UNORM,        D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING         }; break;
    default:
...
uint64_t xxCreateBlendStateD3D12(uint64_t device, char const* sourceColor, char const* operationColor, char const* destinationColor, char const* sourceAlpha, char const* operationAlpha, char const* destinationAlpha)
{
    ...
    desc.RenderTarget[0].BlendOp = d3d12BlendOp(operationColor);
    desc.RenderTarget[0].SrcBlendAlpha = d3d12BlendFactor(sourceAlpha);
    desc.RenderTarget[0].DestBlendAlpha = d3d12BlendFactor(destinationAlpha);
    desc.RenderTarget[0].BlendOpAlpha = d3d12BlendOp(operationAlpha);
    desc.RenderTarget[0].BlendEnable = (desc.RenderTarget[0].SrcBlend != D3D12_BLEND_ONE ||
                                        desc.RenderTarget[0].DestBlend != D3D12_BLEND_ZERO ||
                                        desc.RenderTarget[0].BlendOp != D3D12_BLEND_OP_ADD ||
                                        desc.RenderTarget[0].SrcBlendAlpha != D3D12_BLEND_ONE ||
                                        desc.RenderTarget[0].DestBlendAlpha != D3D12_BLEND_ZERO ||
                                        desc.RenderTarget[0].BlendOpAlpha != D3D12_BLEND_OP_ADD) ? TRUE : FALSE;
