...
uint64_t xxCreateTextureD3D8(uint64_t device, uint64_t format, int width, int height, int depth, int mipmap, int array, void const* external)
{
    ...
    if (depth == 1 && array == 1)
    {
        ...
        HRESULT hResult = d3dDevice->CreateTexture(width, height, mipmap, D3DUSAGE_DYNAMIC, pixelFormat, D3DPOOL_DEFAULT, &d3dTexture);
        if (hResult != S_OK)
            return 0;
    ...
    if (width == height && depth == 1 && array == 6)
    {
        ...
        HRESULT hResult = d3dDevice->CreateCubeTexture(width, mipmap, D3DUSAGE_DYNAMIC, pixelFormat, D3DPOOL_DEFAULT, &d3dCubeTexture);
        if (hResult != S_OK)
            return 0;
    ...
    if (depth != 1 && array == 1)
    {
        ...
        HRESULT hResult = d3dDevice->CreateVolumeTexture(width, height, depth, mipmap, D3DUSAGE_DYNAMIC, pixelFormat, D3DPOOL_DEFAULT, &d3dVolumeTexture);
        if (hResult != S_OK)
            return 0;
...
//------------------------------------------------------------------------------
uint64_t xxCreateRasterizerStateD3D8(uint64_t device, bool cull, bool fill, bool scissor)
{
    D3DRENDERSTATE8 d3dRenderState = {};
