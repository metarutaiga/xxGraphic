...
uint64_t xxCreateTextureD3D11(uint64_t device, uint64_t format, int width, int height, int depth, int mipmap, int array, void const* external)
{
    ...
    {
    case "RGB565"_CC:
    case "BGR565"_CC:   pixelFormat = DXGI_FORMAT_B5G6R5_UNORM;         break;
    case "ARGB1555"_CC:
    case "ABGR1555"_CC: pixelFormat = DXGI_FORMAT_B5G5R5A1_UNORM;       break;
    case "ARGB4444"_CC:
    case "ABGR4444"_CC:
    case "RGBA4444"_CC:
    case "BGRA4444"_CC: pixelFormat = DXGI_FORMAT_B4G4R4A4_UNORM;       break;
    case "RGBA5551"_CC:
    case "BGRA5551"_CC: pixelFormat = DXGI_FORMAT_B5G5R5A1_UNORM;       break;
    case "ARGB8888"_CC:
    case "RGBA8888"_CC: pixelFormat = DXGI_FORMAT_R8G8B8A8_UNORM;       break;
    case "ABGR8888"_CC:
    case "BGRA8888"_CC: pixelFormat = DXGI_FORMAT_B8G8R8A8_UNORM;       break;
    case "DS24"_cc:     pixelFormat = DXGI_FORMAT_D24_UNORM_S8_UINT;    break;
    case "BC1"_cc:
    case "DXT1"_cc:     pixelFormat = DXGI_FORMAT_BC1_UNORM;            break;
    case "BC2"_cc:
    case "DXT3"_cc:     pixelFormat = DXGI_FORMAT_BC2_UNORM;            break;
    case "BC3"_cc:
    case "DXT5"_cc:     pixelFormat = DXGI_FORMAT_BC3_UNORM;            break;
    case "BC4S"_cc:     pixelFormat = DXGI_FORMAT_BC4_SNORM;            break;
    case "BC4U"_cc:
    case "ATI1"_cc:     pixelFormat = DXGI_FORMAT_BC4_UNORM;            break;
    case "BC5S"_cc:     pixelFormat = DXGI_FORMAT_BC5_SNORM;            break;
    case "BC5U"_cc:
    case "ATI2"_cc:     pixelFormat = DXGI_FORMAT_BC5_UNORM;            break;
    case "BC6H"_cc:     pixelFormat = DXGI_FORMAT_BC6H_SF16;            break;
    case "BC7"_cc:      pixelFormat = DXGI_FORMAT_BC7_UNORM;            break;
    default:
    ...
    D3D11_USAGE usage = mipmap > 1 ? D3D11_USAGE_DEFAULT : D3D11_USAGE_DYNAMIC;
    D3D11_CPU_ACCESS_FLAG cpuAccessFlags = mipmap > 1 ? D3D11_CPU_ACCESS_FLAG(0) : D3D11_CPU_ACCESS_WRITE;
...
uint64_t xxCreateVertexShaderD3D11(uint64_t device, char const* shader, uint64_t vertexAttribute)
{
    ...
    if (strcmp(shader, "default") == 0)
    {
        ...
        d3dDevice->CreateVertexShader(vertexShaderCode40, vertexShaderCode40[6], nullptr, &vertexShader);
    ...
    {
        static char const* const macro[] = { "SHADER_HLSL", "11",
                                             "SHADER_VERTEX", "1",
                                             nullptr, nullptr };
        ...
        ID3D11VertexShader* vertexShader = nullptr;
        d3dDevice->CreateVertexShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, &vertexShader);
        memcpy(d3dShader, &vertexShader, sizeof(vertexShader));
...
uint64_t xxCreateFragmentShaderD3D11(uint64_t device, char const* shader)
{
    ...
    if (strcmp(shader, "default") == 0)
    {
        ...
        d3dDevice->CreatePixelShader(pixelShaderCode40, pixelShaderCode40[6], nullptr, &pixelShader);
    ...
    else
    {
        ...
        ID3D11PixelShader* pixelShader = nullptr;
        d3dDevice->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, &pixelShader);
        memcpy(d3dShader, &pixelShader, sizeof(pixelShader));
...
uint64_t xxCreateRasterizerStateD3D11(uint64_t device, bool cull, bool fill, bool scissor)
{
    ...
    desc.FillMode = fill ? D3D11_FILL_SOLID : D3D11_FILL_WIREFRAME;
